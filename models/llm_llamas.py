...
def perform_sentiment_transfer(tokenizer, model, input_sentence, examples, language, task):
    prompt = generate_prompt(input_sentence, examples, language, task)
    # logger.info("Generated prompt for sentiment transfer.")

    inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
    generate_ids = model.generate(**inputs, max_new_tokens=50, pad_token_id=tokenizer.eos_token_id)
...
def process_csv_file(client, args, examples):
    ...
    for index, row in df.iterrows():
        ...
        # try:
        pred_sentence = perform_sentiment_transfer(tokenizer, model, src_sentence, examples, language, task)
